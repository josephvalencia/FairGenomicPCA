setwd("~/FairGenomicPCA/")
library(Matrix)
p = pipe("gunzip -c ALL.chr20.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz  | sed /^#/d  | cut  -f '10-' | ./a.out | cut -f '1-2'")
...
library(threejs)
scatterplot3js(p$u,size=0.15)

# Read just the header of the chromosome file to obtain the sample identifiers
ids = readLines(pipe("gunzip -c ALL.chr20.phase3_shapeit2_mvncall_integrated_v5a.20130502.genotypes.vcf.gz  | sed -n /^#CHROM/p | tr '\t' '\n' | tail -n +10"))
...
scatterplot3js(p$u, col=rainbow(N)[ped$Superpopulation], size=0.15)
