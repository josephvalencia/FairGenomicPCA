...
def nash_social_welfare(B_list,X,log=True,alpha = 0.0):
    ...
    if log:
        welfare = 0
        for group in B_list:
            welfare +=  np.log(np.trace(group @ X))
    else:
        welfare = 1
        for group in B_list:
            welfare *= np.trace(group @ X)  
...
class PCA:
    ...
    def run(self, data : np.ndarray):
        ...
        if self.n_components is not None:
            n,m = corr.shape
            subset = [m-self.n_components,m-1] # n_components largest
            w,v =  eigh(corr,subset_by_index=subset)
        else:
            w,v =  eigh(corr)
      
        descending = np.argsort(-w)
        self.PC = v[:,descending]

class FrankWolfeNash(PCA):


    def run(self,all_data,data_groups,learning_rate,epsilon,alpha=0.5):
        ...
        for name,data in data_groups.items():
            m,n = data.shape
            B = data.T @ data
            self.corr_list.append(B)
            print("group {} cov matrix {}".format(name,B.shape))

        X = self.n_components/n * np.eye(N=n,M=n)
        dual_gap = np.inf
        
        welfare = nash_social_welfare(self.corr_list,X,log=True,alpha=alpha)
        ...
        self.PC =  self.__factors__(X,self.n_components)

    def __factors__(self,X,d):
...
class ParetoPCA(PCA):
    ...
    def track_progress(self):
        ...
        disparities  = []        
        overall_loss = loss_PCA(self.data,self.PC)
        print("overall loss {}".format(overall_loss))

        for name,data_group in self.data_groups.items():
            group_loss = loss_PCA(data_group,self.PC) - self.ideal_loss[name]
